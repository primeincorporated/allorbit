 ...
 * @filesource	favorites/models/favorites_model.php
 */

if ( ! class_exists('Favorites_cacher'))
{
	require_once realpath(rtrim(dirname(__FILE__), "/") . '/../libraries/Favorites_cacher.php');
}
...
class Favorites_Model extends CI_Model
{
	...
	public $form_table_nomenclature		= 'favorites_form_entries_%NUM%';
	...
	public function __construct ()
	{
		...
		$this->root_name 	= strtolower(
			preg_replace("/^Favorites_(.*?)_model/is", '$1', $this->class)
		);
		...
		if ( ! $this->_table)
		{
			...
			$this->_table = 'favorites_' . trim(plural($this->root_name), '_');
		}
	...
	public function insert ($data)
	{
		...
		if ($success)
		{
			Favorites_cacher::clear($this->class);
	...
	//	utilities
	// --------------------------------------------------------------------
	// 	Some functions here are repeated from elsehwere in the Favorites
	...
	{
		return new Favorites_cacher(array($args, $this->_table), $func, $this->class);
	}
	//END cacher
	...
	public function clear_cache ($func = '')
	{
		if (trim($func) !== '')
		{
			Favorites_cacher::clear($this->class, $func);
		}
		else
		{
			Favorites_cacher::clear($this->class);
