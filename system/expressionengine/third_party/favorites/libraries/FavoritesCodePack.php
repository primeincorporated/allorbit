 ...
 * @version		2.0
 * @filesource	favorites/libraries/FavoritesCodePack.php
 */

class FavoritesCodePack
{
	/**
	...
	public $langItems = array(
		...
		'missing_theme_html_exp'		=> 'There should be a folder called \'html\' inside your site\'s \'/themes/solspace_themes/favorites/%code_pack_name%\' folder. Make sure that it is in place and that it contains additional folders that represent the template groups that will be created by this code pack.',
		...
		'missing_code_pack_theme_exp'	=> 'There should be at least one theme folder inside the folder \'%code_pack_name%\' located inside \'/themes/favorites/\'. A theme is required to proceed.',
	...
	public function detectInternalCodePack()
	{
		$this->internalCodePack	= ! (
			get_class($this) === 'FavoritesCodePack' &&
			//why the need for this? Well sometimes PHP was doing
			// (get_class($this) === 'FavoritesCodePack') === true where
		...
		if ( ! empty($folders))
		{
			foreach ($folders as $folder)
			{
				$details = $this->getCodePackDetails(
					$this->addonsFolder . $folder . '/favorites/'
				);
	 ...
	 * /some/path/my_sweet_addon/favorites/ would return: my_sweet_addon
	 *
	 * @access	public
	...
	public function detectNameFromPath($path = '')
	{
		...
		while ($i--)
		{
			if (trim($segs[$i]) == 'favorites' AND $i !== 0)
			{
